  ...
  return (
    ...
    >
      ...
      >
        <title>{title}</title>
        {!hideMonthLabels && (
          <g data-slot="month-labels" className="fill-current">
        ...
        {weeks.map((week, weekIndex) =>
          week.map((activity, dayIndex) => {
            ...
            return (
              // biome-ignore lint/suspicious/noArrayIndexKey: grid cells are addressed by their (week, day) position, there's no other stable id.
...
export const ContributionGraphTotalCount = ({
  ...
  return (
    <div className={cn("text-zinc-500", className)} {...props}>
      {labels.totalCount
        ? labels.totalCount
  ...
  return (
    ...
    >
      ...
      {new Array(maxLevel + 1).fill(undefined).map((_, level) =>
        children ? (
          // biome-ignore lint/suspicious/noArrayIndexKey: `level` is the semantic legend level (0..maxLevel), not an arbitrary array position.
